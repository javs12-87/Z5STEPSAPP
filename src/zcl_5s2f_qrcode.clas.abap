CLASS zcl_5s2f_qrcode DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS ZCL_5S2F_QRCODE IMPLEMENTATION.


  METHOD if_http_service_extension~handle_request.

    DATA(lt_param) = request->get_form_fields( ).
    READ TABLE lt_param REFERENCE INTO DATA(lr_survey) WITH KEY name = 'survey'.
    READ TABLE lt_param REFERENCE INTO DATA(lr_group) WITH KEY name = 'groupid'.

    IF sy-subrc = 0.

      "lv_xml = NEW zcl_fixedasset_integration( )->fetch_data_from_backend( lr_asset_key->value ).
      lv_xml = NEW zcl_qrcode(  )->create_form(
                                  survey  = lr_survey->value
                                  groupid = lr_group->value
                                ).
